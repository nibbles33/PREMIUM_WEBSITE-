#!/usr/bin/env npx tsx
/**
 * QA-only: Grade C Batch D literal claim dump from SOURCE product data.
...
const OUT = path.join(
  __dirname,
  "../docs/qa-screenshots/grade-c-batch-d-2026-09-09/literal-claim-dump.txt",
);
...
function dumpRoute(slug: string): string {
  ...
  out += emitBlock("trustStatement (whoItIsFor / trust band)", config.trustStatement);
...
function main() {
  let out = `GRADE C BATCH D — LITERAL CLAIM DUMP (SOURCE DATA)
Date: 2026-09-09
